...
export function GoogleSignInButton({
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center">
      {/* Official Google Identity Services Render Container - Always mounted to preserve iframe lifecycle */}
      <div
        key="gis-button-container"
        ref={googleBtnContainerRef}
        className={`w-full flex justify-center items-center overflow-hidden min-h-[44px] max-h-[50px] ${
          disabled || isLoading ? "pointer-events-none opacity-40 cursor-not-allowed" : ""
        } ${!gisLoaded || isLoading ? "hidden" : ""}`}
      />

      {/* Loading overlay when authenticating */}
      {isLoading && (
        <div
          key="gis-loading-overlay"
          className="w-full max-w-[360px] h-[44px] py-2 px-4 bg-slate-50 border border-slate-200 text-slate-600 font-semibold text-xs rounded-xl flex items-center justify-center gap-2.5 shadow-2xs"
        >
      ...
      {/* Placeholder while GIS script initializes (without pulse animation) */}
      {!gisLoaded && !isLoading && (
        <div
          key="gis-placeholder-static"
          className="w-full max-w-[360px] h-[44px] py-2 px-4 bg-white border border-slate-300 rounded-lg flex items-center justify-center gap-3 shadow-2xs"
        >
